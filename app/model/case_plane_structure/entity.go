...
var (
	Table       = "case_plane_structure"
	M           = g.DB("default").Model(Table).Safe()
	Table_alias = "case_plane_structure cps"
	M_alias     = g.DB("default").Model(Table_alias).Safe()
)

type Entity struct {
	SerialNumber        string      `orm:"serial_number,size:20,comment:'设备系列号',table_comment:'病例-切面-结构'" json:"serial_number"`
